  ...
  methods =
    ...
    fixPercentages: ->
      ...
      while roundedTotal < 100
        ...
        indexes = []
        i = -1
        while ((i = self.remainders.indexOf(highestRemainder, i + 1)) isnt -1)
          indexes.push(i)
        # Override index value if another characteristic is higher
        if(indexes.length > 1)
          j = 0
          while j < indexes.length
            index = indexes[j] if self.rounded[indexes[j]] > self.rounded[index]
            j++
